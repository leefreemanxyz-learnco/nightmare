...
/**
 * Determine if a selector is inside a container on a page.
 *
 * @param {String} selector
...
exports.withinContainer = function(selector,container,done){
  debug('.withinContainer() for ' + selector)
  this.evaluate_now(function(selector,container){
    var elem = document.querySelector(selector)
    var containerElem = document.querySelector(container)
    if (elem && containerElem){
      var topOfElement = elem.offsetTop
      var bottomOfElement = (elem.offsetTop + elem.offsetHeight)
      var topOfScreen = containerElem.offsetTop
      var bottomOfScreen = (containerElem.offsetTop + containerElem.offsetHeight)
      return ((bottomOfScreen>bottomOfElement)&&(topOfScreen<topOfElement))
    }
    else return false
  }, done, selector, container)
}
